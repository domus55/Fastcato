...
class Player(pygame.sprite.Sprite):
    ...
    def render(self):
        #self.pos[0] = 1200
        self.collider.topleft = self.pos
        # pygame.draw.rect(screen, (255, 0, 0), pygame.Rect(Camera.relativePosition(self.collider.topleft), self.collider.size))
        screen.blit(self.image, Camera.relativePosition((self.pos[0] - 25, self.pos[1] - 65)))
            ...
            for i in range(Player.SHADOW_AMOUNT):
                ...
                if self._shadowFacingRight:
                    ...
                    screen.blit(Player.IMG_SHADOW_RIGHT,
                                Camera.relativePosition(self.shadowImagesPos[i].topleft))
                else:
                    Player.IMG_SHADOW_LEFT.set_alpha(alpha)
                    screen.blit(Player.IMG_SHADOW_LEFT, Camera.relativePosition(self.shadowImagesPos[i].topleft))
    ...
    def _dash(self, keyPressed):
        ...
        if (keyPressed[pygame.K_LSHIFT] or buttons.Buttons.dash) and time.time() > self.last_dash_time + self._DASH_DELAY:
            ...
            if game_info.GameInfo.BUILD_TYPE != game_info.BuildType.WEB:
                t = Timer(self._DASH_DELAY, self._dashReadySound)
                t.start()
            self.last_dash_time = time.time()

    def _dashReadySound(self):
        if main_menu.MainMenu.state is not main_menu.MainMenu.State.CLOSED:
            return
        else:
    ...
    def _isOutOfMap(self):
        ...
        PLAYER_WIDTH = 25
        if self.pos[0] < Camera.borderLeft - PLAYER_WIDTH:
            self.pos[0] = Camera.borderLeft - PLAYER_WIDTH

        if self.pos[0] > Camera.borderRight - PLAYER_WIDTH:
            self.pos[0] = Camera.borderRight - PLAYER_WIDTH
    ...
    def _collisionWithObstacle(self):
        for i in obstacle_manager.ObstacleManager.allObstacles:
