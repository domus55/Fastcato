...
I can handle all click events in here getting rid of unnecessary mess
'''
...
class MainMenu:
    ...
    @staticmethod
    def update():
        cloud_manager.CloudManager.update()

    @staticmethod
    def render():
        if game_info.GameInfo.BUILD_TYPE is not game_info.BuildType.WEB:
            MainMenu.renderBackground()
        screen.screen.blit(MainMenu.image, (600, 187))

        if MainMenu.state is MainMenu.State.IN_SETTINGS:
            for i in range(game_info.GameInfo._sound):
    ...
    def renderBackground():
        ...
        screen.screen.blit(MainMenu.BACKGROUND1, (800, 0))
        cloud_manager.CloudManager.renderBeforeMountains()
        cloud_manager.CloudManager.renderAfterMountains()
